...
from .circles import Circle, norm
from .overlap import get_overlaps_graph, position_to_max_circle_idx
...
def trace_toward_circle(circle, dist_function, epsilon, min_radius, point):
    ...
    while not traced_circles[-1].overlaps(circle):
        ...
        new_centre = (
            traced_circles[-1].centre
            + traced_circles[-1].radius * diff_vector / distance
        )
...
def trace_toward_graph_min(overlaps_graph, dist_function, epsilon, min_radius, point):
    ...
    dists = np.fromiter(
        map(lambda circle: circle.distance_to_point(point), circles), np.float32
    )
    ...
    # steer towards it
    new_circles = trace_toward_circle(
        near_circle, dist_function, epsilon, min_radius, point
    )
...
def trace_toward_graph_all(overlaps_graph, dist_function, epsilon, min_radius, point):
    ...
    new_circles = sum(
        (
            trace_toward_circle(circle, dist_function, epsilon, min_radius, point)
...
def trace_toward_graph_knn(
    overlaps_graph, dist_function, epsilon, min_radius, point, knn=4
):
    circles: list[Circle] = overlaps_graph.vs["circle"]
    # find the nearest circle
    dists = np.fromiter(
    ...
    for near_idx in near_indices:
        ...
        if len(all_new_circles) == 0:
            all_new_circles.append(
                new_circles[0]
            )  # the first circle is placed at the point
